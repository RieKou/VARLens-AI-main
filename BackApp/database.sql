create table public.matches (id bigint generated always as identity primary key, user_id uuid references auth.users(id), name text not null, status text not null default 'uploaded', video_url text not null, result jsonb, created_at timestamptz not null default now());
